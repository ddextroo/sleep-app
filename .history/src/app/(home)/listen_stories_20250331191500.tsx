...
const ListenStories = () => {
  ...
  const handleTogglePlay = async (item) => {
    try {
      ...
      // Load and play new sound using createAsync.
      // If your asset is dynamic, you might need a mapping since Expo's require() doesn't support dynamic paths.
      const { sound: newSound } = await Audio.Sound.createAsync(
        require(assets ? assets[0] : null)
        // or, for a dynamic asset: assets[item.assetIndex]
      );
